...
class TopologyManager(object):
    ...
    def resolve(self):
        ...
        if self.verbose:
            log.info('Starting topology resolution phase')
            log.info('Loading platform engine: {}'.format(self.engine))
        ...
        if self.verbose:
            log.info('Creating platform instance with timestamp: {}'.format(
                timestamp
            ))
        ...
        if self.verbose:
            log.info('Topology resolution phase completed')
    ...
    def build(self):
        ...
        if self.verbose:
            log.info('Starting topology build phase')
        ...
        try:
            ...
            stage = 'add_node'
            if self.verbose:
                log.info('Build stage: add_node - adding nodes to topology')
            for node in self.graph.nodes():
                if self.verbose:
                    log.info('Adding node: {}'.format(node.identifier))
            ...
            for node in self.graph.nodes():
                for port in node.ports():
                    if self.verbose:
                        log.info('Adding port {} to node {}'.format(
            ...
            if self.verbose:
                log.info(
                    'Build stage: add_bilink - creating links between nodes'
                )
            for link in self.graph.links():
                if self.verbose:
                    log.info('Creating link: {}:{} <-> {}:{}'.format(
        ...
        if self.verbose:
            log.info('Topology build phase completed successfully')
    ...
    def unbuild(self):
        ...
        if self.verbose:
            log.info('Starting topology unbuild phase')

        # Remove own reference to enodes
        if self.verbose:
            log.info('Removing references to engine nodes')
        self.nodes = OrderedDict()
        ...
        if self.verbose:
            log.info('Cleaning up platform instance')
        del self._platform
        self._platform = None

        if self.verbose:
            log.info('Topology unbuild phase completed successfully')
