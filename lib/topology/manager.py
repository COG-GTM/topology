...
class TopologyManager(object):
    ...
    def resolve(self):
        ...
        if self.verbose:
            log.info(
                'Topology resolve: Starting resolution with platform "{}"'
                .format(self.engine)
            )
        ...
        if self.verbose:
            log.info(
                'Topology resolve: Platform instance created with timestamp {}'
                .format(timestamp)
            )
        ...
        if self.verbose:
            log.info('Topology resolve: Calling platform resolve method')

        self._platform.resolve()
        self._resolved = True

        if self.verbose:
            log.info('Topology resolve: Resolution completed successfully')
    ...
    def build(self):
        ...
        if self.verbose:
            log.info('Topology build: Starting build process')
        ...
        try:
            stage = 'pre_build'
            if self.verbose:
                log.info('Topology build: Entering stage "pre_build"')
            self._platform.pre_build()
            if self.verbose:
                log.info('Topology build: Stage "pre_build" completed')

            stage = 'add_node'
            if self.verbose:
                log.info('Topology build: Entering stage "add_node"')
            for node in self.graph.nodes():
                if self.verbose:
                    log.info(
                        'Topology build: Adding node "{}"'
                ...
                if self.verbose:
                    log.info(
                        'Topology build: Node "{}" added successfully '
                        '(engine node: "{}")'
                        .format(node.identifier, enode.identifier)
                    )

            if self.verbose:
                log.info(
                    'Topology build: Stage "add_node" completed, '
                    '{} nodes added'.format(len(self.nodes))
                )

            stage = 'add_biport'
            if self.verbose:
                log.info('Topology build: Entering stage "add_biport"')
            port_count = 0
            for node in self.graph.nodes():
                for port in node.ports():
                    if self.verbose:
                        log.info(
                            'Topology build: Adding port "{}" to node "{}"'
                            .format(port.identifier, node.identifier)
                        )
                    ...
                    if self.verbose:
                        log.info(
                            'Topology build: Port "{}" added successfully '
                            '(engine port: "{}")'
                            .format(port.identifier, eport)
                        )

            if self.verbose:
                log.info(
                    'Topology build: Stage "add_biport" completed, '
                    '{} ports added'.format(port_count)
                )

            stage = 'add_bilink'
            if self.verbose:
                log.info('Topology build: Entering stage "add_bilink"')
            link_count = 0
            for link in self.graph.links():
                if self.verbose:
                    log.info(
                        'Topology build: Adding link between "{}:{}" and '
                        '"{}:{}"'.format(
                            link.node1, link.port1, link.node2, link.port2
                        )
                    )
            ...
            if self.verbose:
                log.info(
                    'Topology build: Stage "add_bilink" completed, '
                    '{} links added'.format(link_count)
                )

            stage = 'post_build'
            if self.verbose:
                log.info('Topology build: Entering stage "post_build"')
            ...
            if self.verbose:
                log.info('Topology build: Stage "post_build" completed')
        ...
        if self.verbose:
            log.info(
                'Topology build: Build completed successfully with {} nodes, '
                '{} ports'.format(len(self.nodes), port_count)
            )
    ...
    def unbuild(self):
        ...
        if self.verbose:
            log.info(
                'Topology unbuild: Starting unbuild process with {} nodes'
        ...
        if self.verbose:
            log.info('Topology unbuild: Calling platform destroy hook')

        # Call platform destroy hook
        self._platform.destroy()

        if self.verbose:
            log.info('Topology unbuild: Platform destroy completed')
        ...
        if self.verbose:
            log.info('Topology unbuild: Unbuild completed successfully')
